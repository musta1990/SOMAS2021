...
foreach($arrresultado as $i=>$valor){
    ...
    $titulopagina = "Notificaciones - ".$compania_nombre;
    $descripcionpagina = "Notificaciones - $compania_nombre ";
...
$arrresultado = $conexion->doSelect("notificacion.notif_id, notif_tabla, notificacion.modulo_id, notificacion.elemento_id, 
    ...
    "notificacion
      ...
      ",
    "notif_activo = '1' and notificacion.usuario_iddestino  ='$iniuser' and notificacion.cuenta_id = '$SistemaCuentaId' and notificacion.compania_id = '$SistemaCompaniaId' ", null, "notificacion.notif_id desc");
  ...
  foreach($arrresultado as $i=>$valor){
    ...
    if ($modulo_id=="175"){
      $verinfo = "<a href='panel-solicitud?id=$elemento_id'>$notif_titulo <br><span style='color: #0B787A'>(Ver Solicitud)</span></a>";  
    } else if ($modulo_id=="197"){
      $verinfo = "<a href='panel-chat?id=$elemento_id'>$notif_titulo <br><span style='color: #0B787A'>(Ver Mensaje)</span></a>";  
    } else if ($modulo_id=="198"){
      $verinfo = "<a href='prestador?id=$usuario_iddestino'>$notif_titulo <br><span style='color: #0B787A'>(Ver Calificación)</span></a>";  
    } else{
    ...
    $textonotificaciones .= "
      <tr style='font-size: 14px; text-align: center'>          
          <td>$verinfo</td>            
          <td>$notif_fechareg</td>                   
          <td>$vernotificacion</td>                   
      </tr>
      ";

}

if ($textonotificaciones==""){
    $textonotificaciones .= "
                ...
                <div class="col-md-10 offset-md-1 col-lg-9 offset-lg-0 order-sm-2 order-1">
                  <?php include_once "panel-notificaciones.php"; ?>
                    ...
                    <div class="widget welcome-message" style="padding-bottom: 10px">
                        <h2><i class="fa fa-envelope"></i> Notificaciones del Portal</h2>
                        <p>Puedes observar todas las notificaciones recibidas, estas también fueron enviadas vía email</p>
                    </div>
